...
async function getTableData(cur = 1) {
  let res = await request.get('/projects/learn/ESP/api/log.php?&action=query', {
...
async function handleQueryName(val) {
  ...
  model.value = val
  if (val.length != 0) {
    let res = await request.get("/projects/learn/ESP/api/log.php?&action=query", {event: val})
...
async function dialogConfirm() {
  ...
  if (dialogType.value == 'add') {
    ...
    let res = await request.post("/projects/learn/ESP/api/log.php?action=insert", {...dialogForm.value})
    console.log(res);
    debugView.value = res.data
    //刷新数据
    await getTableData(curPage.value)
    ...
    let res = await request.post("/projects/learn/ESP/api/log.php?action=update", {...dialogForm.value})
    console.log(res);
    // 刷新数据
    await getTableData(curPage.value)
  }
